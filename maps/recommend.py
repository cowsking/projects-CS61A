...
def find_closest(location, centroids):
    ...
    """
    # BEGIN Question 3
    return min([_ for _ in centroids], key = lambda x: distance(location,x))
    # END Question 3
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        centroids = [find_centroid(x) for x in cluster]# END Question 6
        n += 1
    return centroids
...
def rate_all(user, restaurants, feature_fns):
    ...
    def help(res):
        if res in reviewed:
            return user_rating(user, restaurant_name(res))
        else:
            return predictor(res)
    return {restaurant_name(x):help(x) for x in restaurants}
    # END Question 9
...
def search(query, restaurants):
    ...
    """
    # BEGIN Question 10
    return [restaurant for restaurant in restaurants if query in restaurant_categories(restaurant)]        # END Question 10
