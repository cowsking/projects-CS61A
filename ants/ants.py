...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        """
        if (not isinstance(insect, QueenAnt)) or insect.impostor == True:
            if insect.is_ant:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while nearest_place != hive:
            if nearest_place.bees and count >= self.min_range and count <= self.max_range:
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        ...
        if bee != None:
            ...
            self.digestingtime = self.time_to_digest
        # END Problem 10

    def action(self, colony):
        # BEGIN Problem 10
        "*** REPLACE THIS LINE ***"
        if self.digestingtime:
            self.digestingtime -= 1
        else:
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        if self.impostor is True:
            ...
            while place != None:
                if place.ant != None:
                    if place.ant not in self.double_lst:
                        place.ant.damage = place.ant.damage * 2
                        self.double_lst.append(place.ant)
                    if place.ant.container == True and place.ant.ant != None and place.ant.ant not in self.double_lst:
                        place.ant.ant.damage = place.ant.ant.damage * 2
                        self.double_lst.append(place.ant.ant)
                place = place.exit
            self.throw_at(self.nearest_bee(colony.hive))
